...
# scan.yml / docker-push.yml, building from this repo's app/ folder).
# Deliberately separate from the infra roles above: this one only ever
# pushes images, nothing else — no VPC/ALB/IAM/ECS access. Deploying the
# pushed image is a separate, deliberate action, not something this does.
# ---------------------------------------------------------------------------
...
data "aws_iam_policy_document" "ecr_push_assume_role" {
  statement {
    ...
    condition {
      # docker-push.yml lives in this repo (aws-ecs-project) and runs via
      # workflow_run/workflow_dispatch, not a plain push — same
      # repo/wildcard pattern as plan/apply, whose exact sub claim shape
